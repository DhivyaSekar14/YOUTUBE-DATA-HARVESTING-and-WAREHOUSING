...
def Apiconnection():
    api_key = 'AIzaSyC28bMrCHHIPp40gzautFjzBhgunrFnAVM'
...
#data insertion to mongodb and mysql
...
def datatables():
    ...
    videos_table()
    comments_table()
    return "Data inserted and Tables created successfully in MySQL"
...
with st.sidebar:
    select = option_menu("Main Menu", ["Home Page", "Data Extraction and Insertion", "Insights"])
    
if select == "Home Page":
    col1,col2 = st.columns(2,gap= 'medium')
    with col1:
        st.header(":blue[Overview]")
        st.markdown("This app aims to collect and warehouse YouTube data using a combination of MySQL, MongoDB, and Streamlit. The goal is to provide a robust and efficient system for harvesting, storing, and visualizing YouTube-related information.")
        st.header(":blue[Technologies Used]")
        st.write("1. Python")
        st.write("2. Youtube Data API")
        st.write("3. MongoDB")
        st.write("4. MySQL")
    ...
    with col2:
        ...
        st.image(Image.open(r"D:\GUVI\projects\youtube\youtubelogo.png"), width = 500)

if select == "Data Extraction and Insertion":
    tab1,tab2,tab3 = st.tabs(["EXTRACTION", "INSERTION", "TABLE VIEW"])
    
    with tab1:
        st.write("Steps to get the Channel ID:")
        st.write("1. Go to the Channel's home page")
        st.write("2. Click on the right arrow '>' present below the Channel's Name")
        st.write("3. Click on the share channel option")
        st.write("4. Copy the Channel ID")
        col1,col2= st.columns(2)
        with col1:
            channel_id = st.text_input("Enter the YouTube Channel ID:")      
        if st.button("Extract the Data and Upload to MongoDB"):
            ch_ids = []
            db = dbconnection["Youtube_Data"]
            collection1 = db["Channel_Details"]
            for ch_data in collection1.find({},{"_id":0, "Channel_Information":1}):
            ...
            if channel_id in ch_ids:
                st.success("Channel Details of the given Channel ID is already present")
            else:
                insert = channel_details(channel_id)
                st.success(insert)
                st.info("Next: Go to Insertion Tab to transform the data to MySQL")
    with tab2:
        st.write ("Click on the below button to create the table in MySQL")
        if st.button("Insert and Transfrom the data to MySQL"):
            Tables = datatables()
            st.success(Tables)
            st.info ("Next: Go to table view to view the data in tabular format")
    with tab3:        
        select_table = st.radio("Select the Table:",("Channels","Playlists","Videos","Comments"))
...
if select == "Insights":       
    ...
    question = st.selectbox("Choose your question to know the insights", ("1. What are the names of all the videos and their corresponding channels?",
